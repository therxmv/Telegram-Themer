...
function headlineHTML(headline) {
  const inner = headline.parts
    ...
    .join("");
  return `<div style="font-size:92px;line-height:1.32;font-weight:800;color:#0c1a26;letter-spacing:-.025em;text-wrap:pretty;max-width:904px">${inner}</div>`;
}
...
function frameHTML(frame) {
  ...
  return `
  <div style="position:absolute;left:${frame.left}px;top:${frame.top}px;${transform}">
    <div style="position:relative;box-sizing:border-box;width:${frame.width}px;height:${frame.height}px;border-radius:${frame.radius}px;padding:${frame.padding}px;background:#1b2027;box-shadow:0 50px 100px rgba(12,26,38,.28), 0 0 0 1px rgba(12,26,38,.12)">
...
for (const tile of tiles) {
  ...
  const outPath = path.join(outDir, `${tile.id}.png`);
