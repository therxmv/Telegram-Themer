...
| `chat.animateMessageColors` | **Literal boolean.** Whether the outgoing bubble's gradient stops animate (a slow shimmer between `bg` and `gradientBg`) instead of sitting static | `false → false` (Default) / `true → true` (Soza) | Only meaningful when a bubble actually has two distinct gradient stops — see `templates/ios/soza/CLAUDE.md` |
